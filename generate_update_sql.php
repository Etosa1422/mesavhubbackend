<?php
/**
 * Run this locally: php generate_update_sql.php
 * It fetches live rates from bulkfollows and generates a SQL file
 * that you can run in phpMyAdmin to fix all prices on the live server.
 */
...
$lines[] = "-- Run this in phpMyAdmin on your live server";
$lines[] = "-- Updates api_provider_price, rate_per_1000, price for all bulkfollows services";
$lines[] = "";
...
foreach ($dbServices as $s) {
    ...
    $factor = 1 + ($markup / 100);
    $rate_per_1000 = round($newRate * $factor * $conventionRate, 4);
    $price         = round($newRate / 1000 * $factor * $conventionRate, 8);

    $lines[] = "UPDATE services SET api_provider_price = {$newRate}, rate_per_1000 = {$rate_per_1000}, price = {$price} WHERE api_service_id = '{$s->api_service_id}' AND api_provider_id = 1;";
    $count++;
}

$sql = implode("\n", $lines);
file_put_contents(__DIR__ . '/live_price_update.sql', $sql);

echo "Generated {$count} UPDATE statements\n";
echo "File saved: live_price_update.sql\n";
echo "Upload and run this file in phpMyAdmin on your live server.\n";
