...
<head>
  ...
  <title>Affiliate Payout Update</title>
</head>
      ...
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(22,163,74,0.1);">
          ...
          @php
            ...
            $isPaid      = $status === 'paid' || $status === 'approved';
            $isRejected  = $status === 'rejected';
            $badgeBg     = $isPaid ? '#dcfce7' : ($isRejected ? '#fee2e2' : '#fef9c3');
            $badgeColor  = $isPaid ? '#15803d' : ($isRejected ? '#b91c1c' : '#a16207');
            $statusLabel = ucfirst($payout->status);
          @endphp
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#15803d,#16a34a,#22c55e);padding:36px 40px;text-align:center;">
              ...
              <p style="margin:6px 0 0;color:rgba(255,255,255,0.85);font-size:13px;">Affiliate Payout {{ $statusLabel }}</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:40px;">
              ...
              <p style="margin:0 0 28px;font-size:14px;color:#374151;line-height:1.7;">
                @if($isPaid)
                  Great news! Your affiliate payout has been approved and paid out. Please allow 1–3 business days for the funds to reflect in your account.
                @elseif($isRejected)
                  Unfortunately, your affiliate payout request has been rejected. {{ $payout->note ?? 'Please contact our support team for more information.' }}
                @else
                  Your affiliate payout status has been updated.
                @endif
              </p>
              ...
              <p style="margin:0;font-size:13px;color:#6b7280;line-height:1.6;">
                For any questions regarding this payout, please contact our support team.<br /><br />
